...
struct Tac {
    ...
    */
    typedef enum {
        ...
        JZERO,
        PUSH,
		PASS_PARAM, PROCESS_PARAM,
		PUSH_TO_STACK, PUSH_TO_REG,
		CLEAR_ARGS,
        POP,
		CALL,
        RETURN,
    ...
    typedef struct {
        ...
        int order;        // arg order, start with 0
		int total_arg_num;// total arg num (for PASS_PARAM)
        int size;         // stack frame size (for array allocation)
        std::string name; // symbol name (for LoadSymbol tac only)
		int offset;		  // offset in global scope (for Load and Store tac only)
    ...
    static Tac *Pop(Temp dest, int order);
	static Tac *PassParam(Temp src, int order, int total_arg_num);
	static Tac *ProcessParam(Temp src, int order);
    static Tac *PushToReg(Temp src);
	static Tac *PushToStack(Temp src);
	static Tac *ClearArgs(Temp dummy);
	static Tac *Call(Temp dest, Label label, int);
    static Tac *Return(Temp value);
    static Tac *Mark(Label label);
    static Tac *Memo(const char *);
	static Tac *LoadSymbol(Temp addr, std::string name);
	static Tac *Load(Temp dst, Temp addr, int offset);
	static Tac *Store(Temp src, Temp addr, int offset);
	static Tac *Alloc(Temp start_addr, int size);
	static Tac *Memset(Temp start_addr, Temp value, int length);
