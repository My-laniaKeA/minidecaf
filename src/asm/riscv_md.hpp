...
struct RiscvReg {
	...
	int saver;		  // none, caller, callee

    // three constructors for convenience
    RiscvReg(const char *reg_name, bool is_general, int saver);
    RiscvReg() {}
	RiscvReg(RiscvReg *src){
		name = src->name;
		var = src->var;
		dirty = src->dirty;
		general = src->general;
...
struct RiscvInstr : public Instr {
    ...
    enum OpCode {
        ...
        COMMENT,
        // instructions/pseudo instructions in RISC-V
        ADD, SUB, MUL, DIV, REM,
        NEG,
        J,
        BEQZ,
		...
		SLT, SGT,
		SNEZ, SEQZ,
		// pseudo instructions defined in project
		SGE, SLE,	// Set if Greater/Less Equal
		SEQ, SNE,	// Set if Equal/Not Equal
		SOR, SAND,	// Set if Or/And
		ADDI,
		CALL,
		LA, 
...
class RiscvDesc : public MachineDesc {
  ...
  private:
	...
	// translates an Assign TAC into assembly instructions
    void emitAssignTac(RiscvInstr::OpCode, tac::Tac *);
	// translates a PassParam TAC into assembly instructions
	void emitPassParamTac(tac::Tac *);
	// translates a ProcessParam TAC into assembly instructions
	void emitProcessParamTac(tac::Tac *);
	...
	// translates an Alloc TAC into assembly instructions
	void emitAllocTac(tac::Tac *);
	// translates a Memset TAC into assembly instructions
	void emitMemsetTac(tac::Tac *);
	...
	RiscvReg *_old_reg[RiscvReg::TOTAL_NUM]; // registers before function call
	...
	// recovers the content of a register into memory
    void recoverReg(int, LiveSet *);
    ...
    void setParamReg(tac::Tac *t, int cnt);
